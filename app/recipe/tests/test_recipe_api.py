...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_filter_by_tags(self):
        r1 = create_recipe(user=self.user, title="Salad shirazi")
        r2 = create_recipe(user=self.user, title="Season salad")
        tag1 = Tag.objects.create(user=self.user, name="Vegan")
        tag2 = Tag.objects.create(user=self.user, name="Vegeterian")
        r1.tags.add(tag1)
        r2.tags.add(tag2)
        r3 = create_recipe(user=self.user, title="Omelete")
    ...
    def test_filter_by_ingredients(self):
        r1 = create_recipe(user=self.user, title="Sabzi polo mahi")
        r2 = create_recipe(user=self.user, title="joojeh kebab")
        in1 = Ingredient.objects.create(user=self.user, name="Fish")
...
class ImageUploadTests(TestCase):
    ...
    def test_upload_image_bad_request(self):
        """Test uploadin an invalid image"""
        url = image_upload_url(self.recipe.id)
